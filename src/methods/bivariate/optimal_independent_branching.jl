    ...
    while true
        ...
        for r in J, s in J
            ...
            elseif edge_is_diagonal
                edge_is_sw_to_ne =
                    edge_is_diagonal && sign(r[1] - s[1]) == sign(r[1] - s[2])
                subrect_sw_corner = min.(r, s)
                triangle_cuts_se_to_nw =
                    triangle_direction[subrect_sw_corner...]
                if (edge_is_sw_to_ne && triangle_cuts_se_to_nw) || (
                    ...
                    JuMP.@constraint(
                        sub_model,
                        sum(z[j, r, s] for j in 1:t) >= 1
                    )
                else
                    JuMP.@constraint(
                        sub_model,
                        sum(z[j, r, s] for j in 1:t) == 0
                    )
                end
            else
