...
const BivariateSOS2Method =
    Union{K1,OptimalTriangleSelection,NineStencil,SixStencil,UnionJack}

function formulate_pwl!(
    model::JuMP.Model,
    input_vars::NTuple{2,VarOrAff},
    output_vars::NTuple{F,VarOrAff},
    pwl::PWLFunctionPointRep{2,F},
    method::BivariateSOS2Method,
    direction::DIRECTION,
) where {F}
    initPWL!(model)
    ...
    JuMP.@constraint(
        model,
        sum(λ[i, j] * u_1[i] for i in 1:n_1, j in 1:n_2) == input_vars[1]
    )
    JuMP.@constraint(
        model,
        sum(λ[i, j] * u_2[j] for i in 1:n_1, j in 1:n_2) == input_vars[2]
    )
    for k in 1:F
    ...
    for input_seg in canonical_input_segments
        ...
        i_min, i_max =
            extrema([input_seg[1][1], input_seg[2][1], input_seg[3][1]])
        j_min, j_max =
            extrema([input_seg[1][2], input_seg[2][2], input_seg[3][2]])
    ...
    return formulate_triangle_selection!(
        model,
        λ,
        triangle_direction,
        method,
...
function formulate_triangle_selection!(
    ...
    method::BivariateSOS2Method,
    structure::GridTriangulation,
)
    return error(
        "The triangulation structure $structure is not suppported for method $method",
    )
end
