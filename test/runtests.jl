...
const methods_2D_gen = [
    ...
    # OptimalTriangleSelection is tested in a dedicated testset below with a
    # coarser grid to keep CI fast (the selection MIP is expensive).
    [SixStencil(sos2_method) for sos2_method in methods_1D]...,
]

@testset "PiecewiseLinearOpt.jl" begin
    @testset "Simple univariate" begin
        @testset "$method" for method in methods_1D
    ...
    @testset "Simple bivariate" begin
        @testset "$method" for method in methods_2D_gen
            model = Model(optimizer)
    ...
    @testset "2D" begin
        ...
        method_pattern =
            vec(collect(Iterators.product(methods_2D_gen, patterns)))
        k1_methods = [
            (method, :K1) for
            method in [K1(sos2_method) for sos2_method in methods_1D]
        ]
        uj_methods = [
        ...
        @testset "$method, $pattern" for (method, pattern) in method_pattern
            model = Model(optimizer)
            @variable(model, x[1:2])
    ...
    # Test OptimalTriangleSelection with multiple SOS2 methods on a small problem
    # (the small 2-triangle problem is fast; the expensive part is the grid-based test)
    @testset "OptimalTriangleSelection" begin
        @testset "$sos2" for sos2 in [
            ConvexCombination(),
            LogarithmicEmbedding(),
            NativeSOS2(),
        ]
            method = OptimalTriangleSelection(optimizer, sos2)
            model = Model(optimizer)
            @variable(model, x[1:2])
            s1 = PLO.SegmentPointRep{2,1}(
                [(0.0, 0.0), (0.0, 1.0), (1.0, 1.0)],
    ...
    # Test OptimalTriangleSelection on a grid-based 2D problem with a coarser grid
    # to keep CI fast (the selection MIP scales poorly with grid size).
    @testset "OptimalTriangleSelection 2D grid" begin
        @testset "$pattern" for pattern in [:Upper, :K1]
            method = OptimalTriangleSelection(optimizer, ConvexCombination())
            model = Model(optimizer)
            @variable(model, x[1:2])
            # Use a coarser grid (length=4) vs the main 2D tests (length=8)
            d = range(0; stop = 1, length = 4)
            f = (x1, x2) -> 2 * (x1 - 1 / 3)^2 + 3 * (x2 - 4 / 7)^4
            z = piecewiselinear(
                model,
                x[1],
            ...
            optimize!(model)
            @test termination_status(model) == MOI.OPTIMAL
            @test value(z) ≈ objective_value(model) rtol = 1e-3
        end
    end
    ...
    @testset "MultipleChoice" begin
        ...
        s1 = PLO.SegmentHyperplaneRep{1,1}(
            [
                PLO.AffineFunction{1}((1.0,), -1.0),
                PLO.AffineFunction{1}((-1.0,), 2.0),
            ],
    ...
    @testset "Error cases" begin
        ...
        @test_throws ErrorException PLO.SegmentPointRep{1,1}(
            [(1.0,), (2.0,)],
            [(2.5,)],
        )
        ...
        s2 = PLO.SegmentPointRep{1,1}([(2.0,), (3.0,)], [(3.5,), (1.0,)])
        pwl = PLO.PWLFunction([s1, s2], PLO.Intervals())
        @test_throws MethodError piecewiselinear(
            model,
            (x,),
            pwl;
            method = FakeMethod(),
